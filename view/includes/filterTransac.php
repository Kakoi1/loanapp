...
LEFT JOIN accounts acc ON t.acc_id = acc.acc_id WHERE 1=1";
...
if ($searchQuery) {
    $query .= " AND acc_name LIKE :searchQuery";
}
...
if ($searchQuery) {
    ...
    $stmt->bindParam(':searchQuery', $likeQuery);
}
...
foreach ($transactions as $transaction) {
    echo '<tr>';
    echo '<td>' . htmlspecialchars($transaction['acc_name']) . '</td>';
